...
(общую на все проекты). 19.08.2026 на замере 503 страниц одного из сайтов это дало не просто
...
def project_name() -> str:
    ...
    Нужно, потому что лимит бывает не общий, а на конкретный сайт: где экономика
    партнёрская (доход делится), там имеет смысл свой дневной потолок расхода.
    """
